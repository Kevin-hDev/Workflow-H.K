...
📌 CORE PROBLEM
Evaluate project's design maturity across 18 security domains, identify security control gaps
...
⚠️ RISKS
- Incomplete domain assessment (not all 18 domains covered)
...
- Defensive DATA files from P0.loaded_data.defensive READ (not just listed)? [YES/NO]
...
| T3 | Detect extended domain trigger conditions (ext-11→ext-18) | Trigger detection results |
...
| design_matrix contains all 18 domains? | [✅/❌] |
...
| `security_gaps.design_matrix` | BLOCKING - all 18 domains assessed |
...
2. `design_matrix` contains all 18 domains (10 core + 8 extended)
...
> **CRITICAL**: P4 MUST verify all 18 security domains are assessed with proper completeness criteria.
...
domain_coverage_verification:
  ...
  # ============================================================
  # Extended Domains (ext-11 to ext-18) - CONDITIONAL Assessment
  # ============================================================
  ...
  extended_domains:
    ...
    domain_triggers:
      ...
      DESKTOP:  # ext-17: Desktop Framework Security
        trigger_patterns:
          - "tauri.conf.json"
          - "tauri.conf.json5"
          - "src-tauri/"
        ...
        trigger_evidence:
          files_found: []
          detection_method: "No desktop framework patterns detected"
      PRIVESC:  # ext-18: Privilege Escalation & OS Hardening
        trigger_patterns:
          - "pkexec"
          - "sudo"
          - "doas"
          - "polkit"
          - "*.policy"  # polkit policy files
          - "runas"
          - "ShellExecute.*RunAs"
          - "UAC"
...
Evaluate project's design maturity across all 18 security domains, identify gaps between current implementation and security best practices.

---

## Knowledge Reference (Progressive Loading)

1. **Always load**: `security-design.yaml` (18 domains overview)
...
kb_usage_log:
  ...
  kb_available: true
  note: "Extended domains (ext-11 to ext-18) may not have full KB support"
```

**Validation Rules** (GAP-4 Contract):
- **WARNING**: `kb_coverage_percentage < 50%` for core domains (01-10)
- **INFO**: Extended domains (ext-11 to ext-18) queries are optional
...
| ext-17 | DESKTOP | Desktop Framework Security | S, T, I, E |
| ext-18 | PRIVESC | Privilege Escalation & OS Hardening | S, T, E |
...
### ext-17: DESKTOP - Desktop Framework Security

**Trigger**: tauri.conf.json, Electron, sidecar binaries, WebView, PyInstaller

- [ ] Sidecar command injection prevention (allowlist validation)?
- [ ] WebView XSS protection (DOMPurify, CSP)?
- [ ] IPC bridge security (Tauri invoke, events)?
- [ ] Tauri capabilities scoped (deny-by-default)?
- [ ] API keys stored in OS keyring (not code/env)?
- [ ] Memory protection for secrets (zeroize, secrecy)?
- [ ] PyInstaller binary signed and extraction-protected?
- [ ] Code signing on all platforms (Windows/Linux/macOS)?
- [ ] Anti-debug and anti-RE protections?
- [ ] Canary files and deception patterns deployed?
- [ ] BYOK architecture for user API keys?
- [ ] RGPD/privacy data minimization?

### ext-18: PRIVESC - Privilege Escalation & OS Hardening

**Trigger**: pkexec/sudo, UAC elevation, TCC entitlements, setuid/setgid

- [ ] pkexec used instead of sudo (Linux)?
- [ ] polkit policy files properly scoped?
- [ ] UAC bypass vectors mitigated (Windows)?
- [ ] TCC entitlements minimal (macOS)?
- [ ] Signed helper binary for elevation?
- [ ] IPC between elevated/non-elevated authenticated (HMAC)?
- [ ] TLS 1.3 enforced (reqwest/rustls)?
- [ ] Certificate pinning implemented?
- [ ] OS firewall hardening rules?
- [ ] Service hardening baselines?
- [ ] Rollback protection (snapshots, hash chaining)?
- [ ] Supply chain verification (binary signing, SHA-256)?
...
security_gaps:
  ...
  # Design matrix - ALL 18 domains (REQUIRED)
  design_matrix:
    # Core Domains (01-10)
    AUTHN:
      seq: "01"
    ...
    # Extended Domains (ext-11 to ext-18)
    INFRA:
      seq: "ext-11"
  ...
  extended_domains_triggered:
    ...
    - { domain: DESKTOP, trigger: "tauri.conf.json detected" }
    - { domain: PRIVESC, trigger: "pkexec/UAC usage detected" }
```
...
- [ ] YAML is valid with `schema_version: "1.1.0"`
- [ ] `security_gaps.summary` contains all required statistics
- [ ] `security_gaps.design_matrix` has all 18 domains
...
- [ ] Complete assessment matrix (18 domains)
