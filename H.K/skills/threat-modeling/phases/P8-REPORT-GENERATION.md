...
Before generating reports, create the output directory structure:
```bash
# Create audit directory (shared across all security skills)
mkdir -p "{PROJECT_ROOT}/audit/Risk_Assessment_Report/reports"
mkdir -p "{PROJECT_ROOT}/audit/Risk_Assessment_Report/correction"
mkdir -p "{PROJECT_ROOT}/audit/Risk_Assessment_Report/phases"
mkdir -p "{PROJECT_ROOT}/audit/Risk_Assessment_Report/chain"
```
...
2. **Then write 8 reports**: `audit/Risk_Assessment_Report/reports/{PROJECT}-*.md`
3. **Copy Phase reports**: `.phase_working/{SESSION_ID}/reports/P*-*.md → audit/Risk_Assessment_Report/phases/`
4. **Correction plan**: `audit/Risk_Assessment_Report/correction/CORRECTION-PLAN.md`
5. **Chain output**: `audit/Risk_Assessment_Report/chain/threat_model_output.yaml`
...
> **CRITICAL**: Phase 8 requires 8 mandatory reports output to audit/Risk_Assessment_Report/reports/ directory.

### Output Validation

Phase 8 CANNOT complete until:
1. All 8 mandatory reports exist in `audit/Risk_Assessment_Report/reports/`
...
phase_outputs_published:
  - source: ".phase_working/{SESSION_ID}/reports/P1-*.md"
    target: "audit/Risk_Assessment_Report/phases/"
    status: copied
  # ... P2-P7
...
Extract directly from YAML files (LLM analysis):
...
Create all 8 mandatory reports in `audit/Risk_Assessment_Report/reports/`
...
| correction/CORRECTION-PLAN.md | Actionable fix checklist with [ ] checkboxes, files to modify, effort estimates, deadlines | P6+P7 YAML |
...
See: audit/Risk_Assessment_Report/detailed/VR-xxx-*.md
```
...
Copy from `.phase_working/{SESSION_ID}/reports/` to `audit/Risk_Assessment_Report/phases/`:

```bash
cp .phase_working/{SESSION_ID}/reports/P1-PROJECT-UNDERSTANDING.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P2-DFD-ANALYSIS.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P3-TRUST-BOUNDARY.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P4-SECURITY-REVIEW.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P5-STRIDE-THREATS.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P6-RISK-VALIDATION.md audit/Risk_Assessment_Report/phases/
cp .phase_working/{SESSION_ID}/reports/P7-MITIGATION-PLAN.md audit/Risk_Assessment_Report/phases/
```
...
(This creates individual VR-xxx analysis reports in audit/Risk_Assessment_Report/detailed/)
```

If user confirms (or `--detailed` flag was set):
- Read `@phases/P8R-DETAILED-REPORT.md` for instructions
- Generate per-VR detailed reports in `audit/Risk_Assessment_Report/detailed/`
...
**Report Generation**:
- [ ] All 8 reports created in audit/Risk_Assessment_Report/reports/
...
**Chain Output**:
- [ ] threat_model_output.yaml generated in audit/Risk_Assessment_Report/chain/
- [ ] Contains: threat_surfaces, critical_components, stride_findings, recommended_attack_vectors, risk_ratings
- [ ] All VR-xxx mapped to attack vectors for adversary-simulation
...
## Chain Output: threat_model_output.yaml

> **CRITICAL**: After generating all reports, produce `threat_model_output.yaml` in `audit/Risk_Assessment_Report/chain/`.
> This file is the bridge to the next skill in the security chain (adversary-simulation).
> Adversary-simulation's P0 phase will search for this file to target its offensive analysis.

### When to Generate

Generate `threat_model_output.yaml` AFTER all 8 reports are written and validated, as the LAST step of P8.
...
| Field | Source Phase | YAML Key |
|-------|-------------|----------|
| detected_stack | P0 | P0_detection.yaml → detected_stack |
| threat_surfaces | P2+P3 | P2_dfd_elements + P3_boundary_context |
| critical_components | P1+P4 | P1_project_context → module_inventory + P4_security_gaps → gaps |
| stride_findings | P5+P6 | P5_threat_inventory + P6_validated_risks |
| recommended_attack_vectors | P6 | P6_validated_risks → attack_paths + poc_details |
| risk_ratings | P6 | P6_validated_risks → risk_summary |

---

**End of Phase 8 Instructions** (~530 lines, ~5.5K tokens)
