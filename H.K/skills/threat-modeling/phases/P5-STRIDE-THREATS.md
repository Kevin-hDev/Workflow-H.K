...
- Offensive DATA files from P0.loaded_data.offensive READ (not just listed)? [YES/NO]
...
# Read P2/P3/P4 YAML data directly:
...
> Also load: `knowledge/llm-threats.yaml`, `knowledge/agentic-threats.yaml`
...
### Desktop Framework STRIDE Extensions

> When P4 triggers ext-17 (DESKTOP) or ext-18 (PRIVESC), apply these additional threat patterns.

| Desktop Element Type | S | T | R | I | D | E | Special Threats |
|---------------------|---|---|---|---|---|---|-----------------|
| Tauri IPC Bridge    | ✓ | ✓ |   | ✓ |   | ✓ | XSS→RCE via invoke, capability bypass |
| Sidecar Process     | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Command injection, binary replacement |
| WebView             |   | ✓ |   | ✓ |   |   | XSS, content injection, data exfiltration |
| OS Elevation        | ✓ | ✓ | ✓ |   |   | ✓ | UAC bypass, pkexec exploit, TCC bypass |
| Secret Store        | ✓ |   |   | ✓ |   |   | Keyring extraction, memory dump |
| PyInstaller Binary  |   | ✓ |   | ✓ |   | ✓ | Extraction, DLL hijacking, unsigned |

**Desktop-Specific CWE/CAPEC**:
- CWE-78: OS Command Injection → CAPEC-88 (Command Injection via Sidecar)
- CWE-79: XSS → CAPEC-86 (XSS in WebView/IPC Bridge)
- CWE-269: Privilege Escalation → CAPEC-233 (UAC/pkexec/TCC Bypass)
- CWE-426: DLL Search Order Hijacking → CAPEC-471 (DLL Hijacking)
