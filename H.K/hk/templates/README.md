...
This directory contains template files used to initialize H.K workflow outputs when save mode (`-s`) is enabled. This template system significantly reduces token usage by moving repetitive content out of step files.
...
| `{{original_input}}` | Raw user input | `/hk -a -s add auth` |
...
**Output:**
```
.claude/output/hk/01-add-auth-middleware/
